...
ba_ellipse <- function(scan_output, nsim = 1000, coords = coords, cases = cases, pop = pop,
  ...
  #define ein
  ein <- smerc:::nn.cumsum(enn$nn, ex, simplify = FALSE)
  ...
  sim_res <- pbapply::pblapply(seq_len(nsim), function(i){
    ...
    yin = smerc:::nn.cumsum(enn$nn, ysim, simplify = FALSE)
    ...
    clusters <- noc_nn(enn$nn, tobs, nnoc = length(scan_output$clusters)) #noc_nn in smerc
